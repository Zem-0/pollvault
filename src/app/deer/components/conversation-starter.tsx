    ...
    <div className={cn("flex flex-col items-center", className)}>
      <ul className="flex flex-wrap">
        {questions.map((question, index) => (
          ...
          >
            <div
              className="bg-card text-muted-foreground text-sm cursor-pointer rounded-2xl border px-3 py-3 opacity-75 transition-all duration-300 hover:opacity-100 hover:shadow-md"
              onClick={() => {
                onSend?.(question);
              }}
            >
