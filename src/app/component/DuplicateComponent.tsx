...
import { DuplicatePoll, getAllOutLine } from "../api/dashboard/polls";
...
export const DuplicateComponent: React.FC<PageProps> = ({ outlineId, workspaceId }) => {
  ...
  async function duplicatePoll() {
    ...
    try {
      ...
      if (access_token !== null) {
        const responses = await DuplicatePoll(outlineId);
          if (responses?.status === 200) {
            const response = await getAllOutLine(access_token, workspaceId);
            if (response?.status === 200) {
              dispatch(setWorkspaceOutline({ outlines: response.data.user_outlines }));
            }
            showToast({ type: "success", message: "Poll duplicated successfully!" });
          }else{
            showToast({ type: "error", message: "Failed to duplicate poll!" });
          }
      }else {
          ...
          <Dialog className="fixed bottom-0 z-40 " onClose={setOpen}>
            ...
            <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
              <div className="flex min-h-full items-end justify-end p-4 text-center sm:items-end sm:p-0">
                ...
                >
                  <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl mr-12 transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                    <div className="sm:flex sm:items-start">
                      <div className="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-red-100 sm:mx-0 sm:h-10 sm:w-10">
                        <img src="/images/workspace/Illustration.svg" alt="" />
                      </div>
                      <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                        ...
                        Are you sure you want to duplicate the selected poll(s)? 
                        </Text>
  ...
  //                         <div className="mt-2">
  //                           <Text variant="body15R" extraCSS="text-textGray">
  //                           Are you sure you want to duplicate the selected poll? 
  //                           </Text>
  //                         </div>
  //                       </div>
  //                     </div>
